...
Added `CellDecorator`, `LockedCellDecorator`, `CollectableCellDecorator`, `ICellFactory`, and `CellFactory`. Adjusted the `ICell.direction` contract to be explicitly `Direction | undefined` so decorators preserve arrow-cell direction under strict TypeScript settings.
