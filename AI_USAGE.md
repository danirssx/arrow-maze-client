...
Configure the mobile client for EAS cloud builds so Android and iOS can be built through development, preview, and production profiles without committing secrets or enabling unused EAS Update channels. After the initial PR, install EAS CLI locally for the agent session, register/link the Expo EAS project, and document the iPhone usage flow.
...
The user asked to implement Linear ticket `MAZ-213` in a new worktree, reading the client and backend `AGENTS.md`, `MEMORY.md`, and `Linear_MCP_Guideline.md`; register AI usage; validate checks; commit, push, create a PR, and update Linear according to the project agent rules. The user then asked to install EAS, run the registration/linking step directly, and provide a Markdown guide for setup, iPhone usage, and syncing latest changes.
...
- Added `docs/EAS_IPHONE_QUICKSTART.md` with the iPhone usage flow, EAS environment setup, build commands, and Git sync steps.
